...
//         {data && data?.length > 0 ? (
//           <div className='xs:grid-cols-4 mt-2 grid w-full grid-cols-2 gap-4 sm:grid-cols-4 md:grid-cols-8'>
//             {data.map((item) => (
//               <Link
//                 key={item.id}
//                 href={linkClb(
...
const ShopByCardComponent = <TData,>({
  ...
  return (
    <div className='m-auto'>
      ...
      <div className='flex w-full items-center'>
        {data && data?.length > 0 ? (
          <div className='xs:grid-cols-2 mt-2 grid w-full grid-cols-1 gap-4 md:grid-cols-4'>
            {data.map((item) => (
              <Link
                key={item.id}
                href={linkClb(
                ...
                className='bg-shop-light-bg group hoverEffect hover:bg-shop-light-bg flex h-24 flex-row items-center justify-center overflow-hidden rounded-md p-2 text-center'
              >
                {item.images[0] ? (
                ...
                <p className='text-shop-dark-color ml-4 line-clamp-1 text-xs font-semibold'>
                  {item.name}
                </p>
              </Link>
            ))}
